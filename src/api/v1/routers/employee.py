...
@router.post("/create")
async def create_employee(schema: CreateEmployeeRequestSchema,
...
async def generate_employee_invite(employee_id: int,
                                   request: Request,
                                   service: EmployeeService = Depends(),
                                   user_token_schema: UserTokenSchema = Depends(is_user_admin)):
...
async def show_registration_form(email: EmailStr,
                                 service: EmployeeService = Depends()):
    return await service.show_registration_form(email=email)


@router.post("/registration-complete")
async def complete_employee_registration(schema: CompleteEmployeeRegistrationRequestSchema,
                                         service: EmployeeService = Depends()):
    return await service.complete_employee_registration(schema=schema)


@router.patch("/update")
async def update_employee_data(schema: UpdateEmployeeRequestSchema,
                               service: EmployeeService = Depends(),
...
async def rebind_email(schema: RebindEmailRequestSchema,
                       service: EmployeeService = Depends(),
                       user_token_schema: UserTokenSchema = Depends(get_current_user)):
...
async def confirm_rebind_email(schema: ConfirmRebindEmailRequestSchema,
                               service: EmployeeService = Depends(),
                               user_token_schema: UserTokenSchema = Depends(get_current_user)):
